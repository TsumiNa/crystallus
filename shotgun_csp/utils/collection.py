...
class Timer(object):
    ...
    def __repr__(self):
        ...
        return (
            f"Total elapsed: {timedelta(seconds=self.elapsed)} <seconds>\n"
            + "\n".join(
                [
                    f"  |- {k}: {timedelta(seconds=v)}"
                    for k, v in sorted(tmp.items(), key=lambda t: t[1], reverse=True)
                ]
            )
        )
...
class TimedMetaClass(type):
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        for name_, value_ in attrs.items():
            if not name_.startswith("__") and isinstance(
                value_, (types.FunctionType, types.MethodType)
            ):
